...
class SxGeo
{
    ...
    protected function search_idx($ipn, $min, $max): int
    {
        if ($this->batch_mode) {
            while ($max - $min > 8) {
                $offset = ($min + $max) >> 1;
    ...
    public function get_num($ip)
    {
        ...
        if ($this->memory_mode) {
            $result = $this->search_db($this->db, $ipn, $min, $max);
        } else {
            fseek($this->fh, $this->db_begin + $min * $this->block_len);
            $result = $this->search_db(fread($this->fh, $len * $this->block_len), $ipn, 0, $len);
        }

        return $result;
    }

    protected function readData($seek, $max, int $type): array
    {
        $raw = '';
        if ($seek && $max) {
            if ($this->memory_mode) {
    ...
    protected function parseCity($seek, $full = false)
    {
        ...
        if ($full) {
            ...
            $result = ['city' => $city, 'country' => ['id' => $country['id'], 'iso' => $country['iso']]];
        }
    ...
    protected function unpack($pack, $item = ''): array
    {
        ...
        foreach ($pack as $p) {
            ...
            $pos += $l;
            $unpacked[$name] = is_array($v ?? null) ? current($v) : $v;
        }
